...
**User-Agent는 회색지대다.** 현재 값은 일반 Chrome 문자열이고, 실행 환경(리눅스 컨테이너의
헤드리스 Chromium)과 정확히 일치하지 않는다. 정직한 값이라고 말하기는 어렵다.
...
판매완료일 가능성이 높다.** 시세 계산에서 실거래 데이터를 고를 때 이 차이를 감안해야
한다 — 사이트별로 같은 값의 의미가 다르다.
