...
| 테스트 | 실제 Postgres 위에서 실행 (`uv run pytest`, 376건) + 프론트 스모크(`npm run test:web`) |
| 보안 | 로그인 잠금 · 호출 제한(검색·클릭) · 매물 소유 검사 · 비밀값 웹 전용 · DB TLS 모드 — [docs/security.md](docs/security.md) |
...
| [docs/security.md](docs/security.md) | 보안 점검 결과 — 이미 돼 있던 것, 고친 것, 남긴 것, 인프라 인계 |
