...
/**
 * 대문 레일용 — 카테고리별 최신 매물 n개. 전체 최신순 하나로 뽑으면 가방이
 * 도배한다(수집량 비대칭). 다섯 카테고리를 병렬로 찔러 균형을 만들고,
 * 한 카테고리가 실패해도 나머지는 산다.
 */
export async function fetchRecoRail(perCategory = 4) {
  const cats = ["bag", "watch", "jewelry", "apparel", "shoes"];
  const slices = await Promise.all(
    cats.map((cat) =>
      fetch(`${API_BASE}/api/products?category=${cat}&limit=${perCategory}`)
