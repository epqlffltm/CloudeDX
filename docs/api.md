...
- 시도 시각은 성공·실패와 무관하게 남는다. 실패를 안 세면 막힌 검색어를 가장 자주
  두드리게 된다.
- 쿨다운 기록은 **주 DB(writer)** 를 쓴다. RDS 페일오버 중이면 조회는 읽기 복제본으로
  계속 되지만 이 기록은 못 남기는데, 그때 부가 기능 하나 때문에 500을 내보내지 않고
  `failed`로 닫는다. 확인하지 못한 채 외부 조회를 하지도 않는다 — DB가 흔들리는
  동안이야말로 남의 사이트에 요청을 쏟으면 안 되는 때다.

> **쿨다운이 막지 못하는 것**: 검색어를 조금씩 바꿔 가며("샤넬 가방 1", "샤넬 가방 2"…)
> 던지면 매번 새 키라 계속 나간다. 이건 정상 사용자의 연타를 막는 장치이고, IP 단위
> 대량 abuse 는 앞단(WAF rate-based rule)에서 막을 일이다. 코드로 더 복잡하게 풀지 않는다.
