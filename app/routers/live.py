...
from app.config import LIVE_SEARCH_MAX_PAGES, LIVE_SEARCH_TIMEOUT_SECONDS
...
async def live_search(
    ...
    **실패해도 200이다.** 부가 기능이라 오류를 올리면 화면이 이미 보여주고 있는 DB
    ...
    # 같은 검색어가 동시에 여러 번 들어오면 한 번만 친다. 락을 못 잡으면 기다리지
    # 않고 즉시 돌아간다 — 화면은 이미 DB 결과를 보여주고 있으므로 기다릴 이유가
    # 없고, 기다리면 그 시간만큼 커넥션을 붙잡는다.
    async with try_advisory_lock(session, live.lock_key) as acquired:
        if not acquired:
            return LiveSearchResponse(status="skipped", keyword=live.keyword)
        ...
        try:
            async with asyncio.timeout(LIVE_SEARCH_TIMEOUT_SECONDS):
                ...
                saved = await repository.upsert_items(
                    list(collection.items), session=session
                )
