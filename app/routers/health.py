...
async def ready(
    ...
    # 외부 의존성이라 파드를 빼도 장애가 낫지 않지만, 저장소를 못 쓰는 것은 대개
    # **배포 설정 오류**다 — 로컬 모드면 볼륨 권한, S3 모드면 IAM 역할·버킷 이름·
    # 리전. 오케스트레이터가 기동 직후 빨간불로 알려주는 것이 첫 업로드에서 5xx 를
    # 만나는 것보다 낫다. (실제 사례: 업로드 볼륨이 root 소유로 만들어져 사진
    # 업로드가 500 — 테스트는 개발자 PC 권한으로 돌아 잡을 수 없었다.)
    #
