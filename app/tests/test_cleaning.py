...
def test_strips_brand_listing_tail():
    ...
    # 나열 절단 후 절단면의 상태 마커("새상품급 정품(감정O")까지 걷어낸다.
    # 예전에는 "(감정O"가 괄호도 안 닫힌 채 남았다 — 상태 서술은 상품 정보가
    # 아니고, 원본은 raw_title에 보존되므로 정제 제목에서는 뗀다.
    assert strip_spam_tail(title) == "구찌 홀스빗 크로스백/숄더백"
...
def test_detects_non_target_brand():
    ...
    """
    # 발렌시아가는 카테고리 확장 때 수집 대상으로 승격됐다. 잔류 비대상인
    # 펜디로 케이스를 유지한다 — 검사하려는 것은 "비대상 브랜드 판정"이지
    # 특정 브랜드가 아니다.
    title = "펜디 바게트 미니 크로스백 정품S급구찌프라다루이비통디올"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("샤넬 24C 크루즈 자켓 36", "자켓"),
        # 최전방 키워드 규칙: "펜던트"가 "목걸이"보다 앞이라 사유로 잡힌다
        ("구찌 인터로킹 펜던트 목걸이 (실버)", "펜던트"),
        ("에르메스 쇼핑백 4종 세트", "쇼핑백"),
